    ...
    if (hr == DXGI_ERROR_DEVICE_REMOVED || hr == DXGI_ERROR_DEVICE_RESET)
    {
      ...
      sprintf_s(
        ...
        (hr == DXGI_ERROR_DEVICE_REMOVED)
          ? (unsigned)m_d3dDevice->GetDeviceRemovedReason()
          : (unsigned)hr);
...
void
DeviceResources::SetWindow(HWND window, int width, int height)
{
  m_window = window;
...
void
DeviceResources::GetHardwareAdapter(IDXGIAdapter1** ppAdapter)
{
  *ppAdapter = nullptr;
